...
class VacancyControllerTest {
    ...
    void whenRequestVacancyListPageThenGetPageWithVacancies() {
        var vacancy1 = new Vacancy(1, "test1", "desc1", LocalDateTime.now(), true, 1, 2);
        var vacancy2 = new Vacancy(2, "test2", "desc2", LocalDateTime.now(), true, 3, 4);
    ...
    void whenPostVacancyWithFileThenSameDataAndRedirectToVacanciesPage() throws Exception {
        var vacancy = new Vacancy(1, "test", "desc", LocalDateTime.now(), true, 1, 2);
        var fileDto = new FileDto(testFile.getOriginalFilename(), testFile.getBytes());
        var vacancyArgumentCaptor = ArgumentCaptor.forClass(Vacancy.class);
        var fileDtoArgumentCaptor = ArgumentCaptor.forClass(FileDto.class);
        when(vacancyService.save(
                vacancyArgumentCaptor.capture(),
    ...
    @Test
    void whenRequestVacancyPageThenGetPageWithVacancyAndCities() {
        var vacancy = new Vacancy(1, "test", "desc", LocalDateTime.now(), true, 1, 2);
        var city1 = new City(1, "Moscow");
        var city2 = new City(2, "St. Petersburg");
        var expectedCities = List.of(city1, city2);
